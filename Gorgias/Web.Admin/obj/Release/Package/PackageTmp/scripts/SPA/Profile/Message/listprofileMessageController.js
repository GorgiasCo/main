(function (app) {
    'use strict';
    app.controller('listprofileMessageController', ['$scope', '$route', '$http', '$compile', 'apiService', 'DTOptionsBuilder', 'DTColumnBuilder', 'ngAuthSettings', '$location', 'notificationService', function ($scope, $route, $http, $compile, apiService, DTOptionsBuilder, DTColumnBuilder, ngAuthSettings, $location, notificationService) {

        var vm = this;        
        $scope.item = $route.current.params.id;        

        console.log("listprofile Message loaded ;)");
        ...
        $scope.dtColumns = [
            ...
            DTColumnBuilder.newColumn("MessageNote", "Note").withOption('name', 'MessageNote').renderWith(viewHtml),                
            DTColumnBuilder.newColumn("MessageDateCreated", "DateCreated").withOption('name', 'MessageDateCreated'),
            DTColumnBuilder.newColumn("MessageSubject", "Subject").withOption('name', 'MessageSubject'),
            DTColumnBuilder.newColumn("MessageStatus", "Status").withOption('name', 'MessageStatus'),
            DTColumnBuilder.newColumn("MessageIsDeleted", "IsDeleted").withOption('name', 'MessageIsDeleted'),
        ...
        $scope.dtOptions = DTOptionsBuilder.newOptions().withFnServerData(
            function (sSource, aoData, fnCallback, oSettings) {
                $http({
                    method: 'POST',
                    url: ngAuthSettings.apiServiceBaseUri + 'api/Messages/ProfileID/' + ngAuthSettings.userProfileID + '/data',
                    data: {
