    ...
    app.controller('listCityController', ['$scope', '$route', '$http', '$compile', 'apiService', 'DTOptionsBuilder', 'DTColumnBuilder', 'ngAuthSettings', '$location', 'notificationService', function ($scope, $route, $http, $compile, apiService, DTOptionsBuilder, DTColumnBuilder, ngAuthSettings, $location, notificationService) {
        ...
        $scope.dtColumns = [
            //here We will add .withOption('name','column_name') for send column name to the server
                    DTColumnBuilder.newColumn("CityName", "Name").withOption('name', 'CityName').renderWith(viewHtml),                
            DTColumnBuilder.newColumn("CityStatus", "Status").withOption('name', 'CityStatus'),
            DTColumnBuilder.newColumn("CityUpdateDate", "UpdateDate").withOption('name', 'CityUpdateDate'),
        ...
        function deleteRow(item) {
            ...
            if (confirm('Are you sure to delete?')) {
                apiService.deleteItem($scope.baseURL + 'api/City/CityID/' + item, null,
                updateCitySucceded,
                updateCityFailed);
                console.log('Deleted');
            } else {
        ...
        function actionsHtml(data, type, full, meta) {            
            console.log(data);
            return '<a class="btn btn-warning" href="/city/edit/' + full.CityID + '">' +
                '   <i class="fa fa-edit"></i>' +
                '</a>&nbsp;' +
                '<button class="btn btn-danger" ng-click="delete(\'' + full.CityID + '\')">' +
                '   <i class="fa fa-trash-o"></i>' +
                '</button>';
        }
        function viewHtml(data, type, full, meta) {
            return '<a href="/city/' + full.CityID + '">'+ data + '</a>';
        }
        //end datatables ;)
    }]);
