(function (app) {
    'use strict';
    app.controller('addContentRatingController', ['$scope', '$route', '$http', 'apiService', 'ngAuthSettings', '$location', 'notificationService', function ($scope, $route, $http, apiService, ngAuthSettings, $location, notificationService) {

        var vm = this;
        $scope.object = {};
        
        console.log("addContentRatingController loaded ;)");
        $scope.baseURL = ngAuthSettings.apiServiceBaseUri;
        
        $scope.AddObject = insertObject;
        
        $scope.ContentRatingID = 0;
        $scope.hasFile = false;

        function insertObject() {
            apiService.post($scope.baseURL +'api/ContentRating/', $scope.object,
            addContentRatingSucceded,
            addContentRatingFailed);
            console.log("Post!!!" + $scope.object);
        }
     
        function addContentRatingSucceded(result) {
            console.log("Success" + result.data.Result);
            if ($scope.hasFile) {
                //$scope.ContentRatingID = result.data.Result.ContentRatingID;
        ...
        function addContentRatingFailed(response) {
            console.log("Fail");
