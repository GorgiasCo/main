...
namespace Gorgias.DataLayer.Repository.SQL
{
    public class CategoryRepository : ICategoryRepository, IDisposable
    {
        ...
        public IQueryable<Business.DataTransferObjects.Mobile.V2.CategoryMobileModel> GetV2CategoriesAvailableByProfileIDAsQueryable(int ProfileID, string languageCode)
        {
            ...
            //return (from w in context.Categories where (((w.ProfileID == ProfileID) || ((w.AlbumsTopics.Any(m=> m.ProfileID == ProfileID && m.Categories.Any(cc=> cc.CategoryID.Equals(10)))) || (w.AlbumsTopics.Any(m => m.ProfileID == ProfileID && m.Categories.Any(cc => cc.CategoryID.Equals(6)))) || (w.AlbumsTopics.Any(m => m.ProfileID == ProfileID && m.Categories.Any(cc => cc.CategoryID.Equals(5))))) && w.CategoryID !=  29 && w.CategoryDescription == "en") || w.CategoryParentID == 85) orderby w.CategoryType descending, w.CategoryOrder ascending select new Business.DataTransferObjects.Mobile.V2.CategoryMobileModel { Multilanguage = w.ChildCategory.Where(m => m.CategoryDescription == languageCode).FirstOrDefault().CategoryName, CategoryName = w.CategoryName, CategoryID = w.CategoryID, CategoryType = w.CategoryType }).AsQueryable();
            return (from w in context.Categories where ( w.AlbumsTopics.Any(m=> m.ProfileID == ProfileID && m.Categories.Count > 0)|| w.CategoryParentID == 85) orderby w.CategoryType descending, w.CategoryOrder ascending select new Business.DataTransferObjects.Mobile.V2.CategoryMobileModel { Multilanguage = w.ChildCategory.Where(m => m.CategoryDescription == languageCode).FirstOrDefault().CategoryName, CategoryName = w.CategoryName, CategoryID = w.CategoryID, CategoryType = w.CategoryType }).AsQueryable();
        }
