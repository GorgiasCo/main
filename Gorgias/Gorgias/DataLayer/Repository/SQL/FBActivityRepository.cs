...
{   
	public class FBActivityRepository : IFBActivityRepository, IDisposable
	{
		...
		{
			return (from w in context.FBActivities orderby w.FBActivityID descending select w).ToList();
		}
        //List Pagings
        public List<FBActivity> GetFBActivitiesAll(int page = 1, int pageSize = 7, string filter=null)
		{
            ...
            if (filter != null)
            {
                xList = (from w in context.FBActivities orderby w.FBActivityID descending select w).ApplySort(filter).Skip(pageSize * (page - 1)).Take(pageSize).ToList();                
            }
            else {
                xList = (from w in context.FBActivities orderby w.FBActivityID descending select w).Skip(pageSize * (page - 1)).Take(pageSize).ToList();                
            }
            return xList;     
		}
        //IQueryable
		...
		{
			return (from w in context.FBActivities orderby w.FBActivityID descending select w).AsQueryable();
		}
        //IQueryable Pagings
