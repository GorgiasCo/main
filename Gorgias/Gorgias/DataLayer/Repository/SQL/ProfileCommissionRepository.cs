...
{   
	public class ProfileCommissionRepository : IProfileCommissionRepository, IDisposable
	{
		...
		{
			return (from w in context.ProfileCommissions orderby w.ProfileCommissionID descending select w).AsQueryable();
		}
		...
		{
			return (from w in context.ProfileCommissions where w.ProfileID == ProfileID orderby w.ProfileCommissionID descending select w).AsQueryable();
		}
