...
namespace Gorgias.DataLayer.Repository.SQL
{
    public class ProfileTypeRepository : IProfileTypeRepository, IDisposable
    {
        ...
        //CRUD Functions
        public ProfileType Insert(String ProfileTypeName, Boolean ProfileTypeStatus, String ProfileTypeImage, String ProfileTypeDescription, int? ProfileTypeParentID, string ProfileTypeLanguageCode)
        {
            try
            {
                ...
                if (ProfileTypeParentID.HasValue)
                {
                    obj.ProfileTypeParentID = ProfileTypeParentID;
                }
                else
                {
                    obj.ProfileTypeParentID = null;
                }

                obj.ProfileTypeLanguageCode = ProfileTypeLanguageCode;
        ...
        public bool Update(int ProfileTypeID, String ProfileTypeName, Boolean ProfileTypeStatus, String ProfileTypeImage, String ProfileTypeDescription, int? ProfileTypeParentID, string ProfileTypeLanguageCode)
        {
            ProfileType obj = new ProfileType();
        ...
        {
            return (from w in context.ProfileTypes where w.ProfileTypeParentID == null orderby w.ProfileTypeID descending select w).AsQueryable();
        }
        public IQueryable<Business.DataTransferObjects.ProfileTypeDTO> GetProfileTypesAllAsQueryable(string languageCode)
        {
            return (from w in context.ProfileTypes
                    ...
                    select new Business.DataTransferObjects.ProfileTypeDTO
                    {
                        ProfileTypeID = w.ProfileTypeID,
                        ProfileTypeName = w.ProfileTypeName,
                        Multilanguage = w.ProfileTypeChilds.Where(m => m.ProfileTypeLanguageCode == languageCode).FirstOrDefault().ProfileTypeName
                    }).AsQueryable();
        }

        public IQueryable<Business.DataTransferObjects.Mobile.V2.ProfileTypeMobileModel> GetProfileTypesAsQueryable(string languageCode)
        {
            return (from w in context.ProfileTypes
                    where w.ProfileTypeStatus == true && w.ProfileTypeParentID == null
                    orderby w.ProfileTypeName ascending
                    select new Business.DataTransferObjects.Mobile.V2.ProfileTypeMobileModel
                    {
                        ProfileTypeID = w.ProfileTypeID,
                        ProfileTypeName = w.ProfileTypeName,
                        Multilanguage = w.ProfileTypeChilds.Where(m => m.ProfileTypeLanguageCode == languageCode).FirstOrDefault().ProfileTypeName
