(function (app) {
    ...
    function sideBar($timeout, $location, ngAuthSettings, ModalService, authService, apiService, $window, trialService) {
        return {
            restrict: 'AE',
            replace: true,
            scope: {
            ...
            templateUrl: '/layout/directive/sideBarNew.html',
            link: function (scope, element, attr) {
                //scope.profileID = profileID;|| scope.profileTypeID != 5

                scope.getDownloadTrial = trialService.getDownloadTrial();

                scope.timestamp = new Date().getUTCMilliseconds();

                console.log(scope.subscribeMode, 'Subscribe Mode');

                scope.isMiniMenu = false;
                scope.showReach = false;
                console.log(scope.profileTypeID, 'ProfileTypeID')

                scope.$watch('profileTypeID', function (newVal) {
                    // if new value is not null do your all computation
                    console.log(scope.profileTypeID, 'Watch ProfileTypeID new val', newVal);
                    if (newVal == 4 || newVal == 5) {
                        scope.showReach = true;
                        console.log(scope.profileTypeID, 'Watch ProfileTypeID true', scope.showReach);
                    } else {
                        scope.showReach = false;
                        console.log(scope.profileTypeID, 'Watch ProfileTypeID', scope.showReach);
                    }
                });

                scope.$watch('ProfileID', function (newVal) {
                    // if new value is not null do your all computation
                    console.log(scope.ProfileID, 'Watch ProfileID new val', newVal);
                });

                scope.ToggleMiniMenu = function () {
                    scope.isMiniMenu = !scope.isMiniMenu;
                }

                scope.checkedMenu = attr.checkedMenu;
                console.log(scope.activeMenu);
                if (scope.sliderMode != 2) {
                    scope.sliderMode = 1;
                }

                console.log("Yasser" + scope.ProfileID);

                scope.pageMenu = function () {
                    scope.checkedMenu = !scope.checkedMenu;
                    //alert("I like the book!" + scope.ProfileID);
                };

                scope.baseURL = ngAuthSettings.apiServiceBaseUri;
                console.log(scope.baseURL, 'sidebar');

                function loadProfileTypes() {
                    $location.path('/app/' + scope.ProfileURL);

                    //if (authService.authentication.userID > 0) {
                    //    var urlSubscribe = scope.baseURL + 'api/Web/Subscribe/' + authService.authentication.userID + '/' + scope.ProfileID + '/false';
                    //    if (scope.subscribeMode == 'Subscribe') {
                    //        urlSubscribe = scope.baseURL + 'api/Web/Subscribe/' + authService.authentication.userID + '/' + scope.ProfileID + '/true';
                    //    }
                    //    apiService.getAuth(urlSubscribe, null,
                    //    profileTypesLoadCompleted,
                    //    profileTypesLoadFailed, subscribeAuth);
                    //} else {
                    //    showLoginModal();
                    //}
                }
                ...
                function subscribeAuth(error) {
                    showLoginModal();
                }

                function profileTypesLoadCompleted(response) {
                    console.log(response.data.Result);
                    scope.subscribeMode = response.data.Result;
                }
                ...
                //        //        },
                //        //        show: function (slider, trigger) {
                //        //            jQuery('#topdiv').css('display', "block");
                //        //            console.log("Before open!");
                //        //        },
                //        //        hide: function (slider, trigger) {
                //        //            jQuery('#topdiv').css('display', "none");
                //        //            console.log("Before close!");
                //        //        }
                //        //    });
                //        //}

                //        //$("#slider-web").slideReveal({
                //        //    trigger: $(".trigger-web"),
                //        //    position: "right",
                //        //    push: false,
                //        //    overlay: true,
                ...
                //        //        jQuery('#topdiv').removeClass('topdiv').css('top', '');
                //        //        console.log("before ELSE!" + $(this).scrollTop());
                //        //    } else {
                //        //        jQuery('#topdiv').addClass('topdiv')
                //        //            .css('top', 0);
                //        //        console.log("after ELSE! " + $(this).scrollTop());
                //        //    }
                //        //});
                //        /* ---------------------------------------------------------------------------
                //             * Helper Item
                ...
                //                        if (jQuery('body').hasClass('header-rtl')) {
                //                            cHeaderEl.animate({ 'right': -220 }, 500);
                //                        } else {
                //                            cHeaderEl.animate({ 'left': -220 }, 500);
                //                        }
                ...
                //            mfn_stickyH();
                //            console.log('man injammmmmmmmmmm ;)');
                //            mfn_sticky();
                //            onePageActive();
                //        });
