...
angular.module('gorgiasapp')
    .controller('viewMasterAdminProfileController', ['$scope', '$stateParams', '$http', 'apiService', 'ngAuthSettings', '$location', 'notificationService', '$anchorScroll', 'authService', '$translate',
        function ($scope, $stateParams, $http, apiService, ngAuthSettings, $location, notificationService, $anchorScroll, authService, $translate) {
            ...
            $scope.ProfileID = $stateParams.id;// authService.authentication.userID;// $route.current.params.id;
            $scope.editLinkRole = 0;

            console.log($scope.ProfileID, authService.authentication.userUserID, 'login WOW');
            ...
            function checkValidity() {
                ...
                $scope.editLinkRole = authService.authentication.userRole;
            }

            function loadValidity() {
                apiService.get($scope.baseURL + 'api/web/validity/' + authService.authentication.userUserID + '/' + $scope.ProfileID, null,
                ValidityLoadCompleted,
                ValidityLoadFailed);
            }

            function ValidityLoadCompleted(response) {
                console.log(response.data.Result, 'Validity');
            ...
            function ProfileLoadCompleted(response) {
                console.log(response.data.Result, 'Profile Profile Profile ;)');
                $scope.object = response.data.Result;
            }
            ...
            console.log($scope.dzOptions, 'Option WWWW');
